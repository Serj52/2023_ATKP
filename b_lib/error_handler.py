...
class ErrorHandler:
            ...
            if not self.flag_to_stop_timer:
                ...
                if datetime.now() - timedelta(minutes=minutes_wait) >= start:
                    try:
                        ...
                        BusinessPost.send_smtp(self.config.robot_mail, self.config.support_email, subject, body)
    ...
    def restart_block_on_error(self):
        ...
        BusinessPost.send_mail(self.config.support_email, subject, body)
    ...
    def simple_error(self):
        ...
        try:
            ...
            try:
                BusinessPost().send_mail(address=self.config.support_email,
                                         subject=subject,
                                         body=body,
                                         attachments=attach)
            except Exception as err:
                logging.error(f'Ошибка отправки уведомления через Outlook: {err}. Пробую через SMTP.')
                BusinessPost().send_smtp(from_mail=self.config.robot_mail,
